	...
	processCoinStatus: function (message) {
		...
		try {
			...
			db.all(sql, function (err, rows) {
				...
				responseMessage += '\n\n'
				if (foundCoin.notice) {
					if (foundCoin.notice.toLowerCase().indexOf('delisted') !== -1 ||
						foundCoin.notice.toLowerCase().indexOf('remove') !== -1) {
		...
		try {
			db.all(delisterSql, function (err, rows) {
				...
				rows.forEach(function (row) {
					...
					//console.log("Exchange: " + row.exchange + ", last synced: " + row.last_synced + ", notice: " +
					// row.notice);
				})
				...
				for (let i = 0; i < coinStatusesCoins.length; i++) {
					...
					if (coinStatuses[coinStatusesCoins[i]].notice.toLowerCase().indexOf('delist') !== -1 ||
						coinStatuses[coinStatusesCoins[i]].notice.toLowerCase().indexOf('remove') !== -1) {
